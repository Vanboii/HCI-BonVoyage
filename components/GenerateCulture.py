...
def get_llama_culture(city, country, budget, activities, pre_prompt=pre_prompt, prompt_input=prompt_input):
    ...
    prompt_input_updated = prompt_input % (site, city, country, budget, "Theatre & Cultural "+ ", ".join(tag), "Theatre & Cultural") 
        ...
        for d in data:
            ...
            try:
                ...
                if location:
                    ...
                    # ie: when llama has no website given or invalid URL
                    d["place_id"] = place_id
                    try:
...
# activities = ["Theatre & Cultural", "Wheelchair-friendly"]
