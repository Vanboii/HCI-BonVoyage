...
//         const tripToSave = trips[index];
//         setTrips(trips.filter((_, i) => i !== index)); // Remove the trip from community page
//         // Save the trip to local storage
//         const savedTrips = JSON.parse(localStorage.getItem('savedTrips')) || [];
//         localStorage.setItem('savedTrips', JSON.stringify([...savedTrips, tripToSave]));
//     };

//     const filteredItineraries = trips
//         .filter(trip => trip.location.toLowerCase().includes(searchTerm.toLowerCase()))
//         .filter(trip => {
//             const price = parseInt(trip.priceRange.split(' - ')[1].replace('$', ''));
//             return price <= maxBudget;
...
//trying to get the saves to work
import React, { useState } from "react";
import TopBanner from "../../components/banner";
import ButtonComponent from "../../components/button";
import CardComponent from "../../components/card";
...
function CommunityPage() {
    ...
    const handleSave = (index) => {
        ...
        updatedTrips[index].saves += 1; // Increment saves count
        const tripToSave = updatedTrips[index];
        setTrips(updatedTrips.filter((_, i) => i !== index)); // Remove the trip from community page
